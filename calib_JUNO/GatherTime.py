...
def main(order=5, fit_order=10):
    rd, coeff_pe = main_photon_sparse('JUNO_time_test/',order)
    ...
    for i in np.arange(order):
        ...
        plt.legend()
        plt.savefig('coeff_Time_%d_%d_%d.png' % (i, order, fit_order))
        plt.close()
    return coeff_L

coeff_L = main(eval(sys.argv[1]), eval(sys.argv[2]))  
with h5py.File('./Time_coeff_1t_%d_%d.h5' % (eval(sys.argv[1]), eval(sys.argv[2])),'w') as out:
