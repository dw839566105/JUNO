...
x = 17.5 * np.sin(A[:,1]/180*np.pi) * np.sin(A[:,2]/180*np.pi)
y = 17.5 * np.sin(A[:,1]/180*np.pi) * np.cos(A[:,2]/180*np.pi)
...
for i in np.arange(0,10):
    print(i)
    h = tables.open_file(data_path+'1000/%02d.h5' % i)
    #TriggerNo = np.hstack((TriggerNo, h.root.photoelectron[:]['TriggerNo'].astype('int')))
    TriggerNo = h.root.photoelectron[:]['TriggerNo'].astype('int')
    ChannelID = h.root.photoelectron[:]['ChannelID'].astype('int')
    #ChannelID = np.hstack((ChannelID, h.root.photoelectron[:]['ChannelID'].astype('int')))
...
np.sum(PE_total)/20000


# In[328]:


h = tables.open_file('../base1.h5')
base_dif = h.root.base[:]
h.close()
...
for i in np.arange(0,20):
    ...
    for j in np.unique(TriggerNo):
        ...
        PE[j*size:(j+1)*size] = x[A[:,0].astype('int')]
    h.close()
    PE_total0 = np.hstack((PE_total0, PE))


# In[391]:


PE_total10 = np.zeros(0)
size = A[:,0].shape[0]
A[:,0] = A[:,0].astype('int')
for i in np.arange(0,20):
    print(i)
    h = tables.open_file(data_path+'10000/%02d.h5' % i)
    #TriggerNo = np.hstack((TriggerNo, h.root.photoelectron[:]['TriggerNo'].astype('int')))
    TriggerNo = h.root.photoelectron[:]['TriggerNo'].astype('int')
    ChannelID = h.root.photoelectron[:]['ChannelID'].astype('int')
    #ChannelID = np.hstack((ChannelID, h.root.photoelectron[:]['ChannelID'].astype('int')))
    ...
    for j in np.unique(TriggerNo):
        ...
        PE[j*size:(j+1)*size] = x[A[:,0].astype('int')]
    h.close()
    PE_total10 = np.hstack((PE_total10, PE))


# In[394]:


a = np.mean(PE_total10.reshape(-1,40000), axis=1)
b = np.mean(PE_total0.reshape(-1,40000), axis=1)


# In[401]:


plt.hist(z,weights=(b/a))


# In[404]:


plt.scatter(z,b/a,s=0.1,alpha=0.1)


# In[429]:


a = np.mean(PE_total10.reshape(40000, -1),axis=0)
b = np.mean(PE_total0.reshape(40000, -1),axis=0)
plt.scatter(z,(a/b)*100,s=0.1,alpha=0.1)


# In[428]:


index = (z>15) & ((a/b)*100<100)
print(a[index], b[index])
print(np.mean((a/b)[(z>14.9) & (z<15.1)]*100))
...
data = []
for i in np.arange(1000,18000, 1000):
    h = tables.open_file('../JUNO_test/file_%d.h5' % i)
    _ = h.root.coeff25[:]
    #if(i==0):
    #    a = np.sum(_)
    #    _ = np.zeros_like(_)
...
for i in np.arange(30):
    plt.figure(dpi=300)
    plt.plot(np.arange(1,18,1), np.array(data)[:,i],'.')
    plt.title(f'{i}-th order')
    plt.show()
...
for i in np.arange(0,20):
    ...
    h = tables.open_file(data_path+'0/%02d.h5' % i)
    #TriggerNo = np.hstack((TriggerNo, h.root.photoelectron[:]['TriggerNo'].astype('int')))
    TriggerNo = h.root.SimEvent.SimCDHit[:]['event_id'].astype('int')
    ChannelID = h.root.SimEvent.SimCDHit[:]['pmt_id'].astype('int')
    #ChannelID = np.hstack((ChannelID, h.root.photoelectron[:]['ChannelID'].astype('int')))
    #HitPosInWindow = np.hstack((HitPosInWindow, h.root.photoelectron[:]['HitPosInWindow']))

    k = np.zeros(np.size(np.unique(TriggerNo)))
    for index, i in enumerate(np.unique(TriggerNo)):
        k[index] = np.sum(TriggerNo==i)
...
np.sqrt(np.mean(data))/np.mean(data)
