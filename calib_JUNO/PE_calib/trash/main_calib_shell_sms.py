...
def main_Calib(radius,fout, cut_max, PMT_pos):
    ...
    with h5py.File(fout,'w') as out:
        ...
        tmp_x_p, cos_theta = Legendre_coeff(PMT_pos[:,1:4], vertex, cut_max)
        
        print(f'use {time.time() - tmp} s')
        LegendreCoeff = np.tile(tmp_x_p, (EventNo, 1))
        print(PE.shape)
       ...
       ''' 
        ...
        for cut in np.arange(2,cut_max,1): # just take special values
            ...
            model = sm.GLM(y, X, offset=gain, family=sm.families.Poisson())
            result = model.fit()
